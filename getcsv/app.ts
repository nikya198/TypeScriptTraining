...
class CSVFileInput {
  ...
  private createTable() {
    ...
    const eightDayAgoDate = new Date(today);
    eightDayAgoDate.setDate(today.getDate() - 8);

    const day8Data = data.filter((a) => {
      let csvDate = new Date(a[0]);
      return csvDate >= eightDayAgoDate;
    });

    // const rowCount = Math.min(9, data.length); // 最大8行まで処理する
    // const rowsToProcess = data.slice(-rowCount); // 下から指定した行数を取得

    const maxRow = this.getMaxValueRow(day8Data, columnIndex);
